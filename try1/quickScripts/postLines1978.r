...
#'WDOW' #'BCAC' #"CLPR" #"CNRY" #"YTRK" #"CWCD" #"YEYE" #"BLGL" #"GSRK" #"SNOS" #
spp = "BDRK" # 
#
...
for(mcat in mcats){
	...
	#
	lPost = read.csv(sprintf('%s/%s/%s/%s/%s/lpPost.csv', avgPath, ports[1], gears[1], qtrs[1], years[1]))
        sppFlag = spp%in%colnames(lPost)	
	runByYearGear = foreach( g=gears )%dopar%{
		...
		for(y in years){
			...
			#	 
			if( dim(land)[1]>0 & sppFlag ){
				#
				land = as.numeric(land['land'])
				#
         	       		lPost = read.csv(sprintf('%s/%s/%s/%s/%s/lpPost.csv', avgPath, p, g, q, y))[,spp]
...
#
#nowComps[nowComps[,'mcat']==mcat & nowComps[,'year']==y & nowComps[,'qtr']==q & nowComps[,'gear']==g & nowComps[,'port']==p,]
