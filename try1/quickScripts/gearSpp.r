...
mcOut = mclapply( luid, FUN = function(vid){
	...
	for(id in vid){
		#
        	...
        	gear = as.character(D$gear[wid[1]])
        	year = D$year[wid[1]]
        	qtr  = D$qtr[wid[1]]
		#       
...
##WAIC: 287330.74 mlike:-21886.61 DIC:-147634.41
#writeLines('V + f(g:s) MODEL')
#out = inla(weight~species + portComplex + gear + f(QGivenY) + f(interaction(gear, species)), 
...
##WAIC: 42807.15, DIC: 42826.24, mlik: -21887.04 
#writeLines('V + f(g:s) - g - s MODEL')
#out = inla(weight~portComplex + f(QGivenY) + f(interaction(gear, species)), 
#       	Ntrials=DAT$clustSize, family='betabinomial', data=DAT, num.threads=cores, #verbose=T,
#	control.inla=list(
#		#strategy='gaussian',	
#        	#diagonal=0.01,
...
##WAIC: , DIC: , mlik:  
#writeLines('V + f(g:s) - g  MODEL')
#out = inla(weight~species + portComplex + f(QGivenY) + f(interaction(gear, species)), 
#       	Ntrials=DAT$clustSize, family='betabinomial', data=DAT, num.threads=cores, #verbose=T,
#	control.inla=list(
#		#strategy='gaussian',	
#        	#diagonal=0.01,
...
#WAIC: , DIC: , mlik:  
writeLines('V + f(g:s) - s  MODEL')
out = inla(weight~gear + portComplex + f(QGivenY) + f(interaction(gear, species)), 
       	Ntrials=DAT$clustSize, family='betabinomial', data=DAT, num.threads=cores, #verbose=T,
	control.inla=list(
		#strategy='gaussian',	
        	#diagonal=0.01,
...
#write.csv(format(metrics, scientific=T, digits=10), file="./aggMetrics.csv-2mse", row.names=F, quote=F)
