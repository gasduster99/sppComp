...
plotLands = function(..., dirName, yearGoldF=yearGold, comSppYearF=comSppYear, comSppGearYearF=comSppGearYear, sppListF=sppList, col='blue', alpha=50, legend='', calcomCol='black'){
	...
	for(s in sppListF){
		...
		for(k in 1:K){
			...
			toptop = unlist(sapply(as.character(yearGoldF), function(x){ySummary[[x]]['10%',s]}))
			top = unlist(sapply(as.character(yearGoldF), function(x){ySummary[[x]]['25%',s]}))
			med = unlist(sapply(as.character(yearGoldF), function(x){ySummary[[x]]['50%',s]}))
			mean = unlist(sapply(as.character(yearGoldF), function(x){ySummary[[x]][6,s]}))
			blu = comSppYearF[comSppYearF$species==s, 'x']
			bot = unlist(sapply(as.character(yearGoldF), function(x){ySummary[[x]]['75%',s]}))
			botbot = unlist(sapply(as.character(yearGoldF), function(x){ySummary[[x]]['90%',s]}))
			#
	...
	for(g in gearGold){
		...
		for(k in 1:K){
			...
			##10%, 25%, 50%, 75%, 90%, mean
			#sapply(as.character(yearGoldF), function(x){ySummary[[x]]['50%','BCAC']})
			toptop = unlist(sapply(as.character(yearGoldF), function(x){ygSummary[[x]][[g]]['10%',s]}))
			top = unlist(sapply(as.character(yearGoldF), function(x){ygSummary[[x]][[g]]['25%',s]}))
			med = unlist(sapply(as.character(yearGoldF), function(x){ygSummary[[x]][[g]]['50%',s]}))
			mean = unlist(sapply(as.character(yearGoldF), function(x){ygSummary[[x]][[g]][6,s]}))
			blu = comSppGearYearF[comSppGearYearF$species==s & comSppGearYearF$gear==g, 'x']
			bot = unlist(sapply(as.character(yearGoldF), function(x){ygSummary[[x]][[g]]['75%',s]}))
			botbot = unlist(sapply(as.character(yearGoldF), function(x){ygSummary[[x]][[g]]['90%',s]}))
			#
			years = as.numeric(names(toptop))
			if(k==1){
				#       
...
legend = c(legend, subRight(runPaths[1], 32))
print(legend)
#
...
expDistMY = expandDistByMcatYear(land, runPaths, portGold, gearGold, yearGold, qtrGold)
sumDistY = sumDistByYear(expDistMY, yearGold)
summarizeDistY4 = summarizeByYear(sumDistY, yearGold)


#
dirName = '82345M4/' #subRight(runPaths[1], 34)
plotLands(summarizeDistY1, summarizeDistY2, summarizeDistY3, summarizeDistY4, dirName=dirName, col=cols[1:length(legend)], legend=legend)
