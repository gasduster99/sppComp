...
#model
modelDef = weight~species+gear+port+f(YQ)+f(SG)+f(SP)
#, model='iid', hyper=list(prec=list(prior=pPriorTable)))+f(SG, model='iid', hyper=list(prec=list(prior=pPriorTable)))
