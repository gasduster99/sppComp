...
suppressMessages(library(INLA, quietly=TRUE)); INLA:::inla.dynload.workaround()
...
#
sampler = function(model, portGold, gearGold, qtrGold, yearGold, catGold, D, M=10^4, cores=10, samplePath='./'){
	...
	#catGold	: a list 0f gold standard fish ticket landings categories given as integers
	...
	foreach( p=portGold )%dopar%{
		...
		for(lc in catGold){ path = sprintf('./%s/%s/%s/%s/%s/%s/', samplePath, p, g, q, y, lc); dir.create(path)
			#
