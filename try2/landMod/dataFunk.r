...
makeD = function(sppGold, raw){
	...
	colnames(D)[colnames(D)=='marketCategory'] = 'mcat'
	colnames(D)[colnames(D)=='portComplex'] = 'port'
	colnames(D)[colnames(D)=='gearGroup'] = 'gear'
	colnames(D)[colnames(D)=='comLands'] = 'landing'
	...
	for(id in unique(D$id)){
		...
		#each sample should have at least a zero for each species
			...
			#print( raw[raw$port==port & raw$gear==gear & raw$year==year & raw$qtr==qtr & raw$species==sn,'comLands'] )
			#
	                end = end + 1
	                #
	                D$id[end]   	= id
	...
	#
	D = as.data.frame(D)
	D = D[, c('id', 'mcat', 'live', 'year', 'qtr', 'port', 'gear', 'species', 'nBB', 'landing', 'landCat', 'weight')]
	#
	return( D )
}

#
addPredStrat = function(sppGold, portGold, gearGold, yearGold, qtrGold, catGold, D){
