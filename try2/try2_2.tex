...
%For the purpose of fighting against this combinatoric explosion of tests, 
%We take the principled approach of 
...
% Naturally as different likelihoods will introduce different 
...
\end{equation}
%=n~\text{expit}(\theta_{jklm\eta})=n~\text{logit}^{-1}(\theta_{jklm\nu}).\]
...
%global within-cluster correlation. 
...
%bias, while jointly minimizing uncertainty. Janyes \citeyear{jaynes_probability_2003} %(2003, pg. 511) 
...
%At times (particularly when considering random effects) the modeling effort can operate 
%with the oracle of some previously understood correlation %oracle: prior conception 
%structure among categories. 
%
The goal is to %that stepping through possible modeling options for the %$\theta_{jklm\nu}$ in this way should
...
%through a parsimonious model of the linear predictor $\theta_{jklm\nu}$. 
%The process of building the linear predictor is largely bottom-up greedy 
%approach. At times we have some prior conception of the types of parameters 
%that may function better as a fixed or random effect, but 
%
...
\ref{lpAppendix}. 
%In particular, attention is given to the inclusion, or 
...
%%
%After testing numerous iterations of model selection we arrive at the following 
%linear predictor.
%
%\begin{equation}
%\theta_{jklm\nu} = \beta_0 + \beta^{(s)}_j + \beta^{(p)}_k + \beta^{(g)}_l + \beta^{(y:q)}_{m\nu} + \beta^{(s:p)}_{jk}
...
%%
%Firstly, \(\theta\) includes a reference level intercept (\(\beta_0\)).
%Secondly, \(\theta\) is factored broadly among the many strata by fixed 
%effects for each of the species (\(\beta^{(s)}_j\)), port-complexes 
%(\(\beta^{(p)}_k\)), and gear-groups (\(\beta^{(g)}_l\)). Thirdly, 
%year:quarter ($\beta^{(y:q)}_{m\nu}$) and species:port ($\beta^{(s:p)}_{jk}$) 
%two way interactions are included as random effects for the purpose of 
%adaptive partial pooling across space and time. Details about tests run 
%inorder to determine the linear predictor presented here are available in 
%Appendix \ref{lpAppendix}. 
...
%
MCAT 250, Montery, Trawl, 1982, Q2
...
%
The Poisson, Binomial, Negative Binomial, and Beta-Binomial likelihood 
performance was evaluated independently of any other modeling decisions.  

were evto separately each port gearstratum of data from 1978-1990.  
...
\citeyear{nelder_generalized_1989} %McCullagh and Nelder (1989,pg.124)
...
the residual variance. %, while producing more accurate measures of center. 

%EXPAND to explain experiment/INTRODUCE MODELS FOR BELOW
...
%how does $n_ijklm\nu$ enter the equation
\begin{equation}
\mu_{j} = \exp(\theta_{j}). %{\color{red}n_{ij}}
\end{equation}
...
%
%\begin{eqnarray*}
%&\beta^{(t)}_{m\eta} = \beta^{(y)}_{m} + \beta^{(q)}_{\eta} + \beta^{(y:q)}_{m\et
%&\beta^{(y)}_{m} \sim N(0, v^{(y)}) & \\
%&\beta^{(q)}_{\eta} \sim N(0, v^{(q)}) & \\
...
%For the purpose of motivating the time parameter structure to be used across 
%all market categories and time periods, each of models M1-M6 are fit to data 
%from market category 250 from 1978-1982. % in order to motivate the time paramete
%Performing such a model selection exercise across all market categories, in 
%each time period, would not only be impractical but would also risk overfitting 
%models, particually in poorly sampled categories. In the early time periods, 
%market category 250 is the most heavily sampled multi-species market category, 
