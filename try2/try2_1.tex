...
%predictors,
%
%\[\theta_{jklm\nu} = \beta_0 + \beta^{(s)}_j + \beta^{(p)}_k + \beta^{(g)}_l + \
%
%Firstly, \(\theta\) includes a reference level intercept (\(\beta_0\)).
%Secondly, \(\theta\) is factored among the many strata by additive
%offsets from \(\beta_0\) for each of the species (\(\beta^{(s)}_j\)),
%port-complexes (\(\beta^{(p)}_k\)), and gear-groups (\(\beta^{(g)}_l\)).
%Finally year and quarter parameters are indicated generally here inside
%the \(\beta^{(t)}_{m\nu}\) term. Several forms for
%\(\beta^{(t)}_{m\nu}\) are explored each implying a different prior and
%partial pooling strategies as described in the following
%section. %(\ref{riors}).


%For modeling \(\beta^{(t)}_{m\nu}\) we consider a spectrum of models
%which span a wide range of partially pooled models with several
%different predictive structures as seen below.
%
%\subsubsection{(M1)}\label{m1}
%
%\[\beta^{(t)}_{m\nu} = \beta^{(y)}_{m} + \beta^{(q)}_{\nu}\]
...
%pooling among both the \(\beta^{(y)}_{m}\) and \(\beta^{(q)}_{\nu}\)
...
%year.
%
%Furthermore the \(\beta^{(y:q)}_{m\nu}\) are also modeled
%hierarchically to introduce a single variance parameter, \(v\), shared
%among all of the \(m\nu\) time chunks. Although this interaction term
%adds many parameters to the model, the shared \(v\) parameter functions
%to shrink extraneous \(\beta^{(y:q)}_{m\nu}\) estimates back toward the
%common stratum mean.
%
...
%quarterly \(\beta^{(y:q)}_{m\nu}\) interaction terms. This model
...
%\(\beta^{(y:q)}_{m\nu}\) are hierarchically pooled back toward a single
...
%$v$ is a large fixed value) was not considered. The \(\beta^{(y:q)}_{m\nu}\) 
%interaction terms introduce a very large number of parameters, which require 
%regularization in this setting. Furthermore, pooling \(\beta^{(y:q)}_{m\nu}\) 
%through time introduces a formalized way to make predictions about unsampled 
%strata.
%
%% with fixed \(\beta^{(y:q)}_{m\nu}\) interaction terms are
