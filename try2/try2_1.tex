...
\begin{equation}
Y_{ijklm\nu} \substack{i.i.d.\\\sim} f(\theta_{ijklm\nu}, \phi).
\end{equation}

%
Here $\theta_{ijklm\nu}$ is a linear predictor for inferring the mean weight, 
and $\phi$ is a nusiance parameter included (when neccisary) to allow models 
to more flexibly capture higher moments of the \(Y_{ijklm\nu}\). Of particular 
interest, the residual variance is a key value to quantify.

%
Defining a particular form for $f$ also implies the linear predictor/mean 
relationship as well as the structure, and scope, of residual variation. The 
form for $f$ was chosen empirically based upon experimentation amoung the 
poisson, binomial, negative binomial, and beta-binomial distributions (see 
appendix \ref{likelihoodAppendix} for experiment details). Other modeling 
options were considered for the form of $f$, and may possibly represent 
future modeling directions, but the list above was determined to be the most
