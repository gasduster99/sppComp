...
the pragmatic purpose of bridging philosophical divides amoung statisticians 
and more directly communicating our inferencial goals at hand. For further 
discussion see \cite{gelman_bayesian_2013, nelder_generalized_1989}. Further discussion 
of prior structure is developed in section \ref{priors}.
...
the value of the intercept. Fixed effects have a natural interpretation as 
offsets from the reference level. Each of these offsets are assigned diffuse 
normal priors,
...
much more strongly than any other value for any fixed effect, thus the large 
$32^2$ variance. It is this large fixed value of the prior variance 
hyperparameter which produces behavior similar to classical fixed effect 
model, and yet even this prior produces an extremly slight amount of posterior 
shrinkage toward 0, so as to edge on the side of parsimony.

%
When considering random effect priors it is useful to consider how 
overparameterized models may cause overfittingand weaken model performance 
...
bias, while jointly minimizing uncertainty. Janyes (2003, pg. 511) describes 
how the inclusion of estimation bias via the Bayesian methodology may produce 
better performing estimates, more quickly, than unbiased counterparts. Among 
the simplest ways to see the principle is in thei structure of the MSE 
...
%
Random effects are typically groupings of usually many parameters that can be 
...
included random effect recieves the following heirarchical prior structure.
...
%
Similarly to the fixed effects $\beta^{(\text{random})}$ is distributed as a 
normal distribution with its mean at zero. However in contrast to fixed 
effects with their large fixed variance priors, random effects (in the 
Bayesian context) introduce a heirarchical layer to the prior structure around 
the prior variance for $\beta^{(\text{random})}$. The parameter $v$ is 
introduced as a parameter, to be estimated from the data, to determine the 
extent of shrinkage the $\beta^{(\text{random})}$ should experience toward 
zero. As the above model learns the posteriors of the hierarchical 
variance parameter, it affects the degree of shrinkage as well as the effective
