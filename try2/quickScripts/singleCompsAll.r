...
#
for(mct in mcatEff){ portEff = unique(dat[dat$mcat==mct,]$portComplex)
for(plc in portEff){ gearEff = unique(dat[dat$mcat==mct & dat$portComplex==plc,]$gear)
for(ger in gearEff){ yearEff = unique(dat[dat$mcat==mct & dat$portComplex==plc & dat$gear==ger,]$year)
for(yer in yearEff){
