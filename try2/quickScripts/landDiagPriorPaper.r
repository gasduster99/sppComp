...
expandDistByMcatYear = function(landings, runPaths, portGold, gearGold, yearGold, qtrGold){
	...
	for(run in runPaths){
		...
		#
	        ...
	        mySum[[mcat]] = list()
	        mygSum[[mcat]] = list()
	        for(y in as.character(yearEff)){mySum[[mcat]][[y]]=0; mygSum[[mcat]][[y]]=list();
...
where = land$live=='N' & land$year>=minYear & land$year<=maxYear
land = land[where, c('mcat', 'year', 'qtr', 'gear', 'port', 'species', 'weight')]
