...
raw = getRawData(mcat, minYear, maxYear, save=T, fromFile="250CALCOMdata78To82_2024-10-28.csv") #"250data78To83_2024-11-13.csv") #T)
...
regKey = list(
	north = c("CRS", "ERK"),
	noCentral = c("BRG", "BDG", "OSF"),
	soCentral = c("MNT", "MRO"),
	south = c("OSB", "OLA", "OSD")
)
#invert key to name regions based on regKey definition
keyReg = with(stack(regKey), split(as.character(ind), values))
D$region = unlist(sapply(D$port, function(x)keyReg[[x]]))
...
modID = sprintf("%s%sto%sRegPoolPort", regionID, minYear, maxYear)
#
source('model.r')
